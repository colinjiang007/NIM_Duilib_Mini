...
HWND WindowEx::Create(HWND hwndParent, LPCTSTR pstrName, DWORD dwStyle, DWORD dwExStyle, bool isLayeredWindow, const ui::CUiRect& rc)
{
	if (!RegisterWnd())
	{
		return NULL;
...
POINT GetPopupWindowPos(WindowEx* window)
{
	...
	ui::CUiRect rect = window->GetPos(true);
