...
CUiString ShadowWnd::GetSkinFolder()
{
	return _T("shadow");
}

CUiString ShadowWnd::GetSkinFile()
{
	return _T("shadow.xml");
}

CUiString ShadowWnd::GetWindowClassName() const
{
	return _T("ShadowWindow");
}

HWND ShadowWnd::Create(Window* window)
{
	window_ = window;
	return Window::Create(NULL, _T("ShadowWindow"), WS_OVERLAPPEDWINDOW & ~WS_MAXIMIZEBOX, WS_EX_TRANSPARENT | WS_EX_TOOLWINDOW, true);
}
...
HWND ShadowWndBase::Create(HWND hwndParent, LPCTSTR pstrName, DWORD dwStyle, DWORD dwExStyle, 
	bool isLayeredWindow /*= false*/, const ui::CUiRect& rc /*= ui::UiRect(0, 0, 0, 0)*/)
{
