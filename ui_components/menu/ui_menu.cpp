...
void CMenuWnd::Init(STRINGorID xml, LPCTSTR pSkinType, POINT point, PopupPosType popupPosType, bool no_focus)
{
	...
	m_xml = xml;
	no_focus_ = no_focus;
	Create(m_hParent, _T("NIM_DUILIB_MENU_WINDOW"), WS_POPUP, WS_EX_TOOLWINDOW | WS_EX_TOPMOST, true, CUiRect());
...
CUiString CMenuWnd::GetWindowClassName() const
{
	return _T("MenuWnd");
}
...
void CMenuWnd::Show()
{
	...
	CUiRect monitor_rect = oMonitor.rcMonitor;
	ui::CUiSize szInit = { rcWork.right - rcWork.left, rcWork.bottom - rcWork.top };
