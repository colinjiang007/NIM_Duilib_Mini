...
namespace ui
{
	...
	EditWnd::EditWnd() : m_pOwner(NULL), m_hBkBrush(NULL), m_bInit(false), m_bDrawCaret(false)
	{
	}
	...
	LPCTSTR EditWnd::GetWindowClassName() const
	{
		return _T("EditWnd");
	}

	LPCTSTR EditWnd::GetSuperClassName() const
	{
		return WC_EDIT;
	}
	...
	LPCTSTR EditUI::GetClass() const
	{
		return DUI_CTR_EDIT;
	}

	LPVOID EditUI::GetInterface(LPCTSTR pstrName)
	{
		if( _tcscmp(pstrName, DUI_CTR_EDIT) == 0 ) return static_cast<EditUI*>(this);
	...
	HWND EditUI::GetNativeWindow() const
	{
		if (m_pWindow) return m_pWindow->GetHWND();
		return NULL;
	}

	void EditUI::DoEvent(DWORD& event)
	{
		ASSERT(FALSE);
		/*
	...
	void EditUI::SetText(LPCTSTR pstrText)
	{
		m_sText = pstrText;
	...
	HWND EditUI::GetNativeEditHWND() const
	{
	...
	bool EditUI::IsAutoSelAll()
	{
		return m_bAutoSelAll;
	}

	void EditUI::SetAutoSelAll(bool bAutoSelAll)
	{
		m_bAutoSelAll = bAutoSelAll;
	}

	LPCTSTR EditUI::GetNormalImage()
	{
		return m_diNormal.sDrawString;
	}

	void EditUI::SetNormalImage(LPCTSTR pStrImage)
	{
	...
	LPCTSTR EditUI::GetHotImage()
	{
		return m_diHot.sDrawString;	
	}

	void EditUI::SetHotImage(LPCTSTR pStrImage)
	{
	...
	LPCTSTR EditUI::GetFocusedImage()
	{
		return m_diFocused.sDrawString;	
	}

	void EditUI::SetFocusedImage(LPCTSTR pStrImage)
	{
	...
	LPCTSTR EditUI::GetDisabledImage()
	{
		return m_diDisabled.sDrawString;	
	}

	void EditUI::SetDisabledImage(LPCTSTR pStrImage)
	{
	...
	void EditUI::SetSel(long nStartChar, long nEndChar)
	{
		if( m_pWindow != NULL ) Edit_SetSel(*m_pWindow, nStartChar,nEndChar);
	}

	void EditUI::SetSelAll()
	{
		SetSel(0,-1);
	}

	void EditUI::SetReplaceSel(LPCTSTR lpszReplace)
	{
	...
	void EditUI::Move(SIZE szOffset, bool bNeedInvalidate)
	{
