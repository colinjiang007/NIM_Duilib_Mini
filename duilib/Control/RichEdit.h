...
public:
	...
	typedef std::function<bool(LONG, LONG, CUiSize&)> FunGetNaturalSize;
public:
	RichEdit();
	 ...
	 * @return 返回设置的指定位置的内容
	 */
    CUiString GetTextRange(long nStartChar, long nEndChar) const;
	 ...
	 * @return 返回获取的一行数据
	 */
    CUiString GetLine(int nIndex, int nMaxLength) const;
	 ...
	 * @return 返回客户区坐标
	 */
	CUiPoint GetCharPos(long lChar) const;
	 ...
	 * @return 返回客户区坐标
	 */
    CUiPoint PosFromChar(UINT nChar) const;
	 ...
	 */
	int CharFromPos(CUiPoint pt) const;
	...
	CUiSize GetNaturalSize(LONG width, LONG height);
	 ...
	 * @return 无
	 */
    void SetScrollPos(CUiSize szPos);
	 ...
	 * @return 无
	 */
	void PaintCaret(IRenderContext* pRender, const CUiRect& rcPaint);
	 ...
	 */
	bool HittestCustomLink(CUiPoint pt, CUiString& info);
...
protected:
	...
	CUiString m_sCurrentColor;
	CUiString m_sTextColor;
	CUiString m_sDisabledTextColor;
	CUiString m_sPromptColor;
	CUiString m_sCaretColor;
	CUiString m_sText;
